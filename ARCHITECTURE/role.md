...
| `bindings/python/src/types.rs:1091` | `PyRoleConfig` |
| `bindings/python/src/funcs.rs:342` | the three loader functions |
...
`Session::add_agent` (`crates/kerness/src/session.rs:737`) reads the spec far
...
- **`orchestrator.md`** — the whole orchestrator prompt: the layout and every
  ...
  (`crates/kerness/src/session.rs:1270`) supplies only what the harness contract
